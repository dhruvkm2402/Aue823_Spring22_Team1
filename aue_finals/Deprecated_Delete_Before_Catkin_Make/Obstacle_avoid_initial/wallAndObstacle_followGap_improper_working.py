...
def callback_ObsAvd(dt, args):
    ...
    if(args.isObsAvd==True):
        ...
        kw = 0.01                      # Proportional constant for angular velocity control

        #gap detection
        detection1 = list(dt.ranges[-deg_val:])
        detection2 = list(dt.ranges[0:deg_val])
        detection = detection1 + detection2
        gap = []
        ornt = []
        for i in range(len(detection)):
            if(detection[i]>saturated_dist):
                #rospy.loginfo('\ngap more than saturation detected\n')
...
def callback_WlFlw(dt, args):
    ...
    if(args.isObsAvd==False):
        ...
        #    rospy.loginfo("\n switching from wall follower to obstacle avoidance\n" + str(args.isObsAvd))
            ...
            #saturating the max left and right averages
            if(left_avg>0.6):
                left_avg=0.6
            if(right_avg>0.6):
                right_avg=0.6
            ...
            if(front_min<=0.5):
                ...
                elif(thr1 > left_avg - right_avg > margin):
